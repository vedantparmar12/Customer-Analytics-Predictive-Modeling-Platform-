...
class BusinessMetricsCalculator:
    ...
    def calculate_customer_profitability(self, acquisition_cost=50, service_cost_per_order=5):
        ...
        try:
            ...
            # Estimate return cost (based on review scores)
    ...
    def calculate_retention_metrics(self):
        ...
        try:
            ...
            churn_factors = self.df.groupby(['customer_segment', 'clv_tier']).agg({
                'churned': ['mean', 'count'],
                'monetary_value': 'sum'
            }).round(3)
    ...
    def calculate_marketing_metrics(self):
        ...
        try:
            ...
            self.df['discount_responsive'] = (
                self.df.get('is_bargain_hunter', 0) == 1
            ).astype(int)
            
            self.df['loyalty_program_candidate'] = (
                (self.df['customer_segment'].isin(['Champions', 'Loyal Customers'])) |
                (self.df['frequency'] > 5)
            ).astype(int)
    ...
    def create_executive_dashboard_data(self):
        ...
        try:
            ...
            customer_distribution = {
                'by_segment': self.df['customer_segment'].value_counts().to_dict(),
