...
class AdvancedCohortAnalysis:
    ...
    def prepare_cohort_data(self, df=None):
        ...
        try:
            ...
            if df is None:
                ...
                # Merge datasets
                df = orders_df.merge(customers_df[['customer_id', 'customer_unique_id']], on='customer_id')
    ...
    def create_cohort_visualizations(self, retention_matrix, cohort_metrics=None):
        ...
        try:
            ...
            for cohort in retention_matrix.index[:10]:  # Limit to recent 10 cohorts
                fig_curves.add_trace(go.Scatter(
                    x=list(range(len(retention_matrix.columns))),
                    y=retention_matrix.loc[cohort].values,
                    mode='lines+markers',
                    name=str(cohort),
            ...
            periods = list(range(min(13, len(retention_matrix.columns))))
            avg_retention = retention_matrix.iloc[:, :13].mean()
    ...
    def run_complete_analysis(self):
        ...
        try:
            ...
            summary = {
                ...
                'best_cohort': cohort_metrics.loc[cohort_metrics['quality_score'].idxmax(), 'cohort_month'],
