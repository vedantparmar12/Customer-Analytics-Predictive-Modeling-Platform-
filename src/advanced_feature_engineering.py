...
class AdvancedFeatureEngineering:
    ...
    def create_temporal_features(self, df):
        ...
        try:
            ...
            customer_temporal = df.groupby('customer_unique_id').agg({
                'is_weekend': 'mean',
                'order_hour': ['mean', 'std'],
                'time_of_day': lambda x: x.mode()[0] if len(x) > 0 else 'Unknown',
                'is_holiday': 'mean',
                'order_day_of_week': lambda x: x.mode()[0] if len(x) > 0 else 0
            }).reset_index()
    ...
    def create_customer_engagement_features(self, df):
        ...
        try:
            ...
            review_engagement = df.groupby('customer_unique_id').agg({
                ...
                'review_creation_date': lambda x: (pd.to_datetime(x.max()) - pd.to_datetime(x.min())).days if len(x) > 1 else 0,
    ...
    def combine_all_features(self):
        ...
        try:
            ...
            # Load basic customer features
            basic_features = pd.read_csv(f"{self.output_path}/customer_features.csv")
